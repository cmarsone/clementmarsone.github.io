...
Let M1 and M2 be families of DP instances with identical state and action spaces, and let M = M1 $\cup$ M2 represent the set of all instances. An algorithm that finds an optimal action at s0 can differentiate between instances in M1 and M2. We use $Q$ to represent the number of queries a deterministic algorithm makes to the oracle. $\Pi_Q$ is defined as the set of all deterministic algorithms that make at most Q queries to the oracle before returning an optimal action at s0. A randomized algorithm running for at most Q steps is represented by a distribution $\mu$ on $\Pi_Q$.
