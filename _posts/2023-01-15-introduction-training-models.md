...
* Actually, we have to define a loss function $\ell:Y\times\mathbb{R}^k \to \mathbb{R}\char{+}$ \& try to minimize it over a distribution by finding $\theta=\text{argmin}_\theta \mathbb{E}_{\mathbb{P}(X,Y)}(\ell(y, \sigma_\theta(x))$
